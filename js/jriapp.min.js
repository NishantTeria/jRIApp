...
style:{classes:e?e:null},position:{my:"top left",at:"bottom right",target:b,viewport:$(a.window),adjust:{method:"flip none",x:0,y:0}},hide:{fixed:!0,delay:250}};b.data("qtip")?c?b.qtip("option","content.text",c):b.qtip("destroy"):c&&b.qtip(e)},hasProp:function(b,c){if(!b)return!1;if(b.hasOwnProperty(c))return!0;if(Object===b)return!1;var e=Object.getPrototypeOf(b);return m.hasProp(e,c)},createDeferred:function(){return a.$.Deferred()},cloneObj:function(b,c){var e,a,n;if(!b)return b;if(h.Array(b)){n=
b.length;e=Array(n);for(a=0;a<n;a+=1)e[a]=m.cloneObj(b[a],null)}else if(h.SimpleObject(b)){e=c||{};var g,d=Object.getOwnPropertyNames(b);n=d.length;for(a=0;a<n;a+=1)g=d[a],e[g]=m.cloneObj(b[g],null)}else return b;return e},shallowCopy:function(b){return m.mergeObj(b,{})},mergeObj:function(b,c){c||(c={});if(!b)return c;for(var e=Object.getOwnPropertyNames(b),a,n=0,g=e.length;n<g;n+=1)a=e[n],c[a]=b[a];return c},removeFromArray:function(b,c){var e=b.indexOf(c);-1<e&&b.splice(e,1);return e},insertIntoArray:function(b,
...
h.push(n));if(d)f.push(d),p.push(d);else for(;0<h.length;)n=h.pop(),d||(d=n),d.lastWins?0==f.length&&f.push(n):f.push(n),p.push(n);try{if(0<f.length){for(g=a.length;0<g;)g-=1,-1<p.indexOf(a[g])&&a.splice(g,1);f.forEach(function(r){try{r.action.apply(e._owner,r.args)}catch(b){e._owner._onError(b,e)}})}}finally{0==a.length&&(delete this._queue[b],this._owner.removeOnPropertyChange(b,this.uniqueID))}}}},enQueue:function(b){b=m.extend(!1,{prop:"",groupName:null,predicate:null,action:null,actionArgs:[],
...
!0;var t=function(){throw Error("getFunc is not emplemented!");},f,x=k[0].name;r?(b.isReadOnly=!1,d._childAssocMap[k[0].childToParentName]=k[0],k[0].fieldRels.forEach(function(a){a=d._fieldMap[a.childField];!b.isReadOnly&&a.isReadOnly&&(b.isReadOnly=!0)}),t=function(){return d.dbContext.getAssociation(x).getParentItem(this)},b.isReadOnly||(d._trackAssoc[x]=k[0],f=function(b){var c,e,r=d.dbContext.getAssociation(x);if(b&&!r.parentDS.entityType.isPrototypeOf(b))throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_PARAM_INVALID_TYPE,
"value",r.parentDS.entityType._typeName));if(b&&b._isNew)this._setFieldVal(a.fieldName,b._key);else if(b){c=0;for(e=r.childFldInfos.length;c<e;c+=1)this[r.childFldInfos[c].fieldName]=b[r.parentFldInfos[c].fieldName]}else{this._getFieldVal(a.fieldName)&&this._setFieldVal(a.fieldName,null);c=0;for(e=r.childFldInfos.length;c<e;c+=1)this[r.childFldInfos[c].fieldName]=null}})):(d._parentAssocMap[k[0].parentToChildrenName]=k[0],t=function(){return d.dbContext.getAssociation(x).getChildItems(this)});q[a.fieldName]=
{set:f,get:t}},g=function(a){q[a.fieldName]={set:function(b){this._setFieldVal(a.fieldName,b)},get:function(){return this._getFieldVal(a.fieldName)}}};b.forEach(function(b){if(h.hasProp(p,b.fieldName))throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_ITEM_NAME_COLLISION,a,b.fieldName));if(b.isCalculated){var c=d.getFieldInfo(b.fieldName);c.isClientOnly=!0;c.isReadOnly=!0;c={dbSetName:a,fieldName:b.fieldName,getFunc:null};d.dbContext._onGetCalcField(c);h.check_is.Function(c.getFunc)&&(b.dependentOn&&k(b),
q[b.fieldName]={get:c.getFunc})}else b.isNavigation?f(b):g(b)});q._dbSet={get:function(){return this.__dbSet}};this._entityType=p.extend({__dbSet:d},q,function(b){b._typeName=a+"Entity"});this._mapAssocFields()},_fillFromService:function(a){a=h.extend(!1,{res:{names:[],rows:[],pageIndex:null,pageCount:null,dbSetName:this.dbSetName,totalCount:null},isPageChanged:!1,fn_beforeFillEnd:null},a);var b=this,c=a.res,e=c.names,d=c.rows||[],q=this._entityType,t=[],f=[],g=[],n=[],l=this.isPagingEnabled,m=this.query,
u=!0,w;this._onFillStart({isBegin:!0,rowCount:d.length,time:new Date,isPageChanged:a.isPageChanged});try{m&&(u=m.isClearPrevData,m.isClearCacheOnEveryLoad&&m._clearCache(),u&&this.clear(),m._reindexCache(),1<m.loadPageCount&&l&&(w=m._getCache(),m.isIncludeTotalCount&&!h.check_is.nt(c.totalCount)&&(w.totalCount=c.totalCount)));g=d.map(function(a){var c=a.key;if(!c)throw Error(RIAPP.ERRS.ERR_KEY_IS_EMPTY);var r=b._itemsByKey[c];r?a.values.forEach(function(a,b){r._refreshValue(a,e[b],1)}):(w&&(r=w.getItemByKey(c)),
r?a.values.forEach(function(a,b){r._refreshValue(a,e[b],1)}):r=q.create(a,e));return r});if(m&&(m.isIncludeTotalCount&&!h.check_is.nt(c.totalCount)&&(this.totalCount=c.totalCount),1<m.loadPageCount&&l)){w.fillCache(c.pageIndex,g);var p=w.getCachedPage(m.pageIndex),g=p?p.items:[]}g.forEach(function(a){var b=this._itemsByKey[a._key];b?n.push(b):(this._items.push(a),this._itemsByKey[a._key]=a,t.push(a),f.push(this._items.length-1),n.push(a))},this);0<t.length&&(this._onItemsChanged({change_type:k.ADDED,
items:t,pos:f}),this.raisePropertyChanged("count"));a.fn_beforeFillEnd&&a.fn_beforeFillEnd()}finally{this._onFillEnd({isBegin:!1,rowCount:n.length,time:new Date,resetUI:u,fetchedItems:n,newItems:t,isPageChanged:a.isPageChanged})}this.moveFirst();return{fetchedItems:n,newItems:t,isPageChanged:a.isPageChanged}},_fillFromCache:function(a){a=h.extend(!1,{isPageChanged:!1,fn_beforeFillEnd:null},a);var b=[],c=[],e=this.query;if(!e)throw Error(String.format(RIAPP.ERRS.ERR_ASSERTION_FAILED,"query is not null"));
...
k,f=a.parameters,q=f.length,t,g,n;c||(c={});for(k=0;k<q;k+=1){t=f[k];g=c[t.name];if(!t.isNullable&&!t.isArray&&t.dataType!==b.String&&h.check_is.nt(g))throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_SVC_METH_PARAM_INVALID,t.name,g,a.methodName));if(h.check_is.Function(g))throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_SVC_METH_PARAM_INVALID,t.name,g,a.methodName));t.isArray&&(!h.check_is.nt(g)&&!h.check_is.Array(g))&&(g=[g]);n=null;h.check_is.Array(g)?(n=[],g.forEach(function(a){n.push(e.stringifyValue(a,
t.dateConversion,self._serverTimezone))}),n=JSON.stringify(n)):n=e.stringifyValue(g,t.dateConversion,self._serverTimezone);d.paramInfo.parameters.push({name:t.name,value:n})}return d},_invokeMethod:function(a,b,c){var e=this,k,q;this.isBusy=!0;var t=function(a){try{if(!a)throw Error(h.format(RIAPP.ERRS.ERR_UNEXPECTED_SVC_ERROR,"operation result is undefined"));f.checkError(a.error,"invoke");c({result:a.result,error:null})}catch(b){d._checkIsDummy(b)||(e._onDataOperError(b,"invoke"),c({result:null,
...
f=a.isPagingEnabled,g=a.dbSetName,n=this.getDbSet(g);this.waitForNotSubmiting(function(){n.waitForNotLoading(function(){var l=n.query,m=c._getUrl("GetItems"),w,u,p=function(){n.isLoading=!1;c.isBusy=!1},v=function(a){p();e.resolve(a)},A=function(a){p();c._onDataOperError(a,"load");e.reject()},z;n.isLoading=!0;c.isBusy=!0;try{a.pageIndex=q;c._beforeLoad(a,l,n);var y,l=1;if(1<k&&f){if(a._isPageCached(q)){z=c._loadFromCache(a,b);z.outOfBandData=null;v(z);return}y=a._getCache().getNextRange(q);q=y.start;
l=y.cnt}w={dbSetName:g,pageIndex:q,pageSize:a.pageSize,pageCount:l,isIncludeTotalCount:a.isIncludeTotalCount,filterInfo:a.filterInfo,sortInfo:a.sortInfo,paramInfo:c._getMethodParams(a._queryInfo,a.params).paramInfo,queryName:a.queryName};u=JSON.stringify(w);h.performAjaxCall(m,u,!0,function(a){var e=[],q,k;try{q=a.indexOf(d.modules.consts.CHUNK_SEP),-1<q?(e.push(a.substr(0,q)),e.push(a.substr(q+d.modules.consts.CHUNK_SEP.length))):e.push(a),e=e.map(function(a){return JSON.parse(a)}),k=e[0],1<e.length&&
(k.rows=e[1]),z=c._onLoaded(k,b),z.outOfBandData=a.extraInfo,v(z)}catch(r){A(r)}},function(a){p();c._onDataOperError(a,"load");e.reject()},null)}catch(D){A(D)}},[],!0,b?"paging":null)},[],b?"paging":null);return e.promise()},getDbSet:function(a){if(!this.isInitialized)throw Error(RIAPP.ERRS.ERR_DOMAIN_CONTEXT_NOT_INITIALIZED);var b=this._dbSets[a];if(!b)throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_DBSET_NAME_INVALID,a));return b},getAssociation:function(a){var b=this._assoc[a];if(!b)throw Error(RIAPP.utils.format(RIAPP.ERR_ASSOC_NAME_INVALID,
...
a});k=p.extend({_init:function(){var a=d.$(this.el);a.addClass(s.rowCollapsed);a.addClass(s.rowExpander)},_onCellClicked:function(){this._row&&(this._super(),this._row.isExpanded=!this._row.isExpanded)},_toggleImage:function(){var a=d.$(this.el);this._row.isExpanded?(a.removeClass(s.rowCollapsed),a.addClass(s.rowExpanded)):(a.removeClass(s.rowExpanded),a.addClass(s.rowCollapsed))},toString:function(){return"ExpanderCell"}},null,function(a){f.ExpanderCell=a});n=p.extend({_create:function(a,b){this._isEditing=
...
this.$div;b.removeClass(s.colSortable).removeClass(s.colSortAsc).removeClass(s.colSortDesc);switch(a){case "ASC":b.addClass(s.colSortAsc);break;case "DESC":b.addClass(s.colSortDesc);break;default:this.isSortable&&b.addClass(s.colSortable)}this._sortOrder=a},get:function(){return this._sortOrder}}},function(a){f.DataColumn=a});y=p.extend({_init:function(){this._super();this.$div.addClass(s.rowExpander)},toString:function(){return"ExpanderColumn"}},null,function(a){f.ExpanderColumn=a});w=p.extend({_init:function(){this._super();
