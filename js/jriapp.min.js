...
k.push(n));if(d)p.push(d),m.push(d);else for(;0<k.length;)n=k.pop(),d||(d=n),d.lastWins?0==p.length&&p.push(n):p.push(n),m.push(n);try{if(0<p.length){for(g=a.length;0<g;)g-=1,-1<m.indexOf(a[g])&&a.splice(g,1);p.forEach(function(c){try{c.action.apply(e._owner,c.args)}catch(b){e._owner._onError(b,e)}})}}finally{0==a.length&&(delete this._queue[b],this._owner.removeOnPropertyChange(b,this.uniqueID))}}}},enQueue:function(b){b=p.extend(!1,{prop:"",groupName:null,predicate:null,action:null,actionArgs:[],
...
b):c},_getTgtChangedFn:function(a,b,c,e,d){return function(n,s){var f=a._app.parser._resolveProp(b,c);0<e.length&&a._checkBounded(null,"target",d,e);a._parseTgtPath(f,e,d)}},_getSrcChangedFn:function(a,b,c,e,d){return function(n,s){var f=a._app.parser._resolveProp(b,c);0<e.length&&a._checkBounded(null,"source",d,e);a._parseSrcPath(f,e,d)}},_parseSrcPath:function(a,b,c){this._sourceObj=null;0===b.length?this._sourceObj=a:this._parseSrcPath2(a,b,c);this._targetObj&&this._updateTarget()},_parseSrcPath2:function(a,
...
_removeIsBusy:function(a){this._busyTimeOut&&(clearTimeout(this._busyTimeOut),this._busyTimeOut=null);(a=this._app._getElView(a))&&g.isPrototypeOf(a)&&a.destroy()},_loadTemplate:function(){var a=this,b=a._templateID,l,n=!1;a._promise&&(a._promise.reject("cancel"),a._promise=null);a._unloadTemplate();b&&(b=a._loadTemplateElAsync(b),n="pending"==b.state(),a._promise=d.createDeferred(),b.done(function(b){a._promise&&a._promise.resolve(b)}),b.fail(function(b){a._promise&&a._promise.reject(b)}),n&&(a._el=
l=f.document.createElement("div"),a._appendIsBusy(l),a._promise.done(function(){a._removeIsBusy(l)}),a._promise.fail(function(b){"cancel"==b&&a._removeIsBusy(l)})),a._promise.then(function(b){a._isDestroyCalled||(a._promise=null,b?(n&&a._el.appendChild(b),a._el=b,a._lfTime=a._app._bindTemplateElements(a._el),(b=a._getTemplateElView())&&b.templateLoaded(a),a._updateBindingSource()):a._unloadTemplate())},function(b){if(!a._isDestroyCalled&&(a._promise=null,"cancel"!=b)){var e;b&&(b.message?e=b:b.statusText?
e=Error(b.statusText):d.check_is.String(b)&&(e=Error(b)));e||(e=Error(String.format(RIAPP.ERRS.ERR_TEMPLATE_ID_INVALID,a._templateID)));a._app._onError(e,a)}}))},_updateBindingSource:function(){var a,b,d,n=this._getBindings();a=0;for(b=n.length;a<b;a+=1)d=n[a],d.isDisabled=this._isDisabled,d.isSourceFixed||(d.source=this._dctxt)},_updateIsDisabled:function(){var a,b,d,n=this._getBindings(),f=this._getElViews(),g=this._app._getElViewType(h.ELVIEW_NM.DATAFORM);a=0;for(b=n.length;a<b;a+=1)d=n[a],d.isDisabled=
this._isDisabled;a=0;for(b=f.length;a<b;a+=1)d=f[a],g.isPrototypeOf(d)&&d.form&&(d.form.isDisabled=this._isDisabled)},_unloadTemplate:function(){try{if(this._el){var a=this._templElView;this._templElView=void 0;a&&a.templateUnloading(this)}}finally{this._lfTime&&(this._lfTime.destroy(),this._lfTime=null),this._el&&f.$(this._el).remove(),this._el=null}},destroy:function(){this._isDestroyed||(this._busyTimeOut&&(clearTimeout(this._busyTimeOut),this._busyTimeOut=null),this._promise&&(this._promise.reject("cancel"),
this._promise=null),this._dctxt=null,this._unloadTemplate(),this._templateID=null,this._templElView=void 0,this._super())},findElByDataName:function(a){return f.$(this._el).find(["*[",h.DATA_ATTR.DATA_NAME,'="',a,'"]'].join("")).toArray()},findElViewsByDataName:function(a){var b=this,d=[];this.findElByDataName(a).forEach(function(a){(a=b._app._getElView(a))&&d.push(a)});return d},toString:function(){return"Template"}},{dataContext:{set:function(a){this._dctxt!==a&&(this._dctxt=a,this.raisePropertyChanged("dataContext"),
...
s=!0,p;this._onFillStart({isBegin:!0,rowCount:d.length,time:new Date,isPageChanged:a.isPageChanged});try{u&&(s=u.isClearPrevData,u.isClearCacheOnEveryLoad&&u._clearCache(),s&&this.clear(),u._reindexCache(),1<u.loadPageCount&&m&&(p=u._getCache(),u.isIncludeTotalCount&&!k.check_is.nt(c.totalCount)&&(p.totalCount=c.totalCount)));n=d.map(function(a){var c=a.key;if(!c)throw Error(RIAPP.ERRS.ERR_KEY_IS_EMPTY);var d=b._itemsByKey[c];d?a.values.forEach(function(a,b){d._refreshValue(a,e[b],1)}):(p&&(d=p.getItemByKey(c)),
d?a.values.forEach(function(a,b){d._refreshValue(a,e[b],1)}):d=q.create(a,e));return d});if(u&&(u.isIncludeTotalCount&&!k.check_is.nt(c.totalCount)&&(this.totalCount=c.totalCount),1<u.loadPageCount&&m)){p.fillCache(c.pageIndex,n);var v=p.getCachedPage(u.pageIndex),n=v?v.items:[]}n.forEach(function(a){var b=this._itemsByKey[a._key];b?h.push(b):(this._items.push(a),this._itemsByKey[a._key]=a,f.push(a),g.push(this._items.length-1),h.push(a))},this);0<f.length&&(this._onItemsChanged({change_type:l.ADDED,
items:f,pos:g}),this.raisePropertyChanged("count"));a.fn_beforeFillEnd&&a.fn_beforeFillEnd()}finally{this._onFillEnd({isBegin:!1,rowCount:h.length,time:new Date,resetUI:s,fetchedItems:h,newItems:f,isPageChanged:a.isPageChanged})}this.moveFirst();return{fetchedItems:h,newItems:f,isPageChanged:a.isPageChanged}},_fillFromCache:function(a){a=k.extend(!1,{isPageChanged:!1,fn_beforeFillEnd:null},a);var b=this,c=[],e=[],d=this.query;if(!d)throw Error(String.format(RIAPP.ERRS.ERR_ASSERTION_FAILED,"query is not null"));
...
l,f=a.parameters,q=f.length,x,g,n;c||(c={});for(l=0;l<q;l+=1){x=f[l];g=c[x.name];if(!x.isNullable&&!x.isArray&&x.dataType!==b.String&&k.check_is.nt(g))throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_SVC_METH_PARAM_INVALID,x.name,g,a.methodName));if(k.check_is.Function(g))throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_SVC_METH_PARAM_INVALID,x.name,g,a.methodName));x.isArray&&(!k.check_is.nt(g)&&!k.check_is.Array(g))&&(g=[g]);n=null;k.check_is.Array(g)?(n=[],g.forEach(function(a){n.push(e.stringifyValue(a,
x.dateConversion,self._serverTimezone))}),n=JSON.stringify(n)):n=e.stringifyValue(g,x.dateConversion,self._serverTimezone);d.paramInfo.parameters.push({name:x.name,value:n})}return d},_invokeMethod:function(a,b,c){var e=this,l,q;this.isBusy=!0;var g=function(a){try{if(!a)throw Error(k.format(RIAPP.ERRS.ERR_UNEXPECTED_SVC_ERROR,"operation result is undefined"));f.checkError(a.error,"invoke");c({result:a.result,error:null})}catch(b){d._checkIsDummy(b)||(e._onDataOperError(b,"invoke"),c({result:null,
...
f=a.isPagingEnabled,g=a.dbSetName,n=this.getDbSet(g);this.waitForNotSubmiting(function(){n.waitForNotLoading(function(){var h=n.query,m=c._getUrl("GetItems"),u,s,p=function(){n.isLoading=!1;c.isBusy=!1},v=function(a){p();e.resolve(a)},A=function(a){p();c._onDataOperError(a,"load");e.reject()},z;n.isLoading=!0;c.isBusy=!0;try{a.pageIndex=q;c._beforeLoad(a,h,n);var y,h=1;if(1<l&&f){if(a._isPageCached(q)){z=c._loadFromCache(a,b);z.outOfBandData=null;v(z);return}y=a._getCache().getNextRange(q);q=y.start;
h=y.cnt}u={dbSetName:g,pageIndex:q,pageSize:a.pageSize,pageCount:h,isIncludeTotalCount:a.isIncludeTotalCount,filterInfo:a.filterInfo,sortInfo:a.sortInfo,paramInfo:c._getMethodParams(a._queryInfo,a.params).paramInfo,queryName:a.queryName};s=JSON.stringify(u);k.performAjaxCall(m,s,!0,function(a){var e=[],q;try{q=a.indexOf(d.modules.consts.CHUNK_SEP),-1<q?(e.push(a.substr(0,q)),e.push(a.substr(q+d.modules.consts.CHUNK_SEP.length))):e.push(a),e=e.map(function(a){return JSON.parse(a)}),setTimeout(function(){var d,
q=e[0],l=!!q.included&&0<q.included.length;try{if(1<e.length&&(d=e[1])&&0<d.length)if(l&&q.included.forEach(function(a){a.rows=d.splice(0,a.rowCount);if(a.rowCount!=a.rows.length)throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_ASSERTION_FAILED,"subset.rowCount == subset.rows.length"));}),q.rows=d,q.rowCount!=q.rows.length)throw Error(RIAPP.utils.format(RIAPP.ERRS.ERR_ASSERTION_FAILED,"getDataResult.rowCount == getDataResult.rows.length"));z=c._onLoaded(q,b);z.outOfBandData=a.extraInfo;v(z)}catch(t){A(t)}},
