...
value:p})}return c},_invokeMethod:function(a,b,c){var d=e.DATA_OPER.INVOKE,l,h;this.isBusy=!0;var k=function(a){try{if(!a)throw Error(g.format(RIAPP.ERRS.ERR_UNEXPECTED_SVC_ERROR,"operation result is undefined"));g.checkError(a.error);c({result:a.result,error:null})}catch(b){f._checkIsDummy(b)||(this._onDataOperError(b,d),c({result:null,error:b}))}};try{l=JSON.stringify(b),h=this._getUrl(e.DATA_SVC_METH.Invoke),g.performAjaxCall(h,l,!0,function(a){k(a);this.isBusy=!1},function(a){k({result:null,error:a});
this.isBusy=!1},this)}catch(n){f._checkIsDummy(n)&&f._throwDummy(n),this.isBusy=!1,this._onDataOperError(n,d),c({result:null,error:n}),f._throwDummy(n)}},_getEventNames:function(){var a=this._super();return["submit_error","define_calc"].concat(a)},_loadFromCache:function(a,b){var c=e.DATA_OPER.LOAD,d=a._dbSet,g;try{g=d._fillFromCache({isPageChanged:b,fn_beforeFillEnd:null})}catch(l){f._checkIsDummy(l)&&f._throwDummy(l),this._onDataOperError(l,c),f._throwDummy(l)}return g},_loadIncluded:function(a){var b=
